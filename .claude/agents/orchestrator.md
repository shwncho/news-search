...
2. `.claude/rules/` 하위 세 파일을 읽어 컨벤션 기준을 숙지한다
...
prompt: 다음 파일들을 리뷰하세요 (구현 파일 + 테스트 파일 전체):
        {STAGE 1 생성 파일 목록}
        {STAGE 2 생성 파일 목록}
        CLAUDE.md, .claude/rules/ 하위 모든 컨벤션 기준으로 분석하고
...
### 누락 고려사항 체크
- [ ] `DomainEvent` 마커 인터페이스가 실제로 사용되는 Domain Event가 있을 때만 생성되었는지 (미사용 시 생성 금지)
