...
class OrderService(
    ...
    fun createOrder(request: CreateOrderRequest): CreateOrderResponse {
        val order = Order.create(CustomerId.of(request.customerId))
...
- `application/dto`의 Request/Response DTO를 직접 사용, `presentation/` 레이어에 별도 DTO 생성 금지
...
네이밍: `{Action}Request` / `{Action}Response`

```kotlin
// Request: 입력 검증 어노테이션 포함
data class CreateOrderRequest(
    @field:NotBlank val customerId: String,
    @field:NotEmpty val items: List<CreateOrderItemRequest>,
)
...
- Request DTO: `@field:` prefix로 Bean Validation 어노테이션 적용
